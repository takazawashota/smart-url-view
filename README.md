...
- **画像キャッシュ**: 外部画像を`wp-content/uploads/smart-url-view/`にダウンロード・保存
- **画像リサイズ**: WordPressの画像エディタで自動的に200x200pxにリサイズ
...
キャッシュをクリアする場合は、プラグインを一度無効化して再度有効化してください。これにより：
- データベースのトランジェントキャッシュがクリア
- `wp-content/uploads/smart-url-view/`内の画像キャッシュが削除

または、FTP/SFTPで`wp-content/uploads/smart-url-view/`ディレクトリを手動で削除することもできます。
